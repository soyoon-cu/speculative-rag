...
class FAISSIndex:
    ...
    def load(cls, index_path: str | Path, meta_path: str | Path) -> FAISSIndex:
        ...
        texts, titles = _load_metadata(meta_path)

        logger.info("Loaded FAISS index with %d vectors from %s", index.ntotal, index_path)

        return cls(index, texts, titles)


def _load_metadata(meta_path: str | Path) -> tuple[list[str], list[str]]:
    ...
    if path.is_dir():
        ...
        return _metadata_from_arrow_dataset(dataset, path)

    with open(path, "rb") as fh:
        meta = pickle.load(fh)

    return meta["texts"], meta["titles"]


def _metadata_from_arrow_dataset(dataset: Dataset, path: Path) -> tuple[list[str], list[str]]:

    columns = set(dataset.column_names)

    if {"texts", "titles"} <= columns:
        texts = dataset["texts"]

        titles = dataset["titles"]

    elif {"text", "title"} <= columns:
        texts = dataset["text"]
        ...
        raise ValueError(
            f"Unsupported Arrow metadata columns in {path}: {dataset.column_names}. "
            "Expected texts/titles or text/title."
        )

    logger.info("Loaded Arrow passage metadata with %d rows from %s", len(dataset), path)

    return list(texts), list(titles)


def _create_faiss_index(device: str) -> faiss.Index:
    ...
    if device.startswith("cuda") and hasattr(faiss, "StandardGpuResources"):
        try:
            ...
            logger.warning("GPU FAISS unavailable; building CPU FAISS index.")
...
def build_index(
    ...
    """Embed passages and write FAISS flat-IP index to disk.

    The full DPR corpus is large enough that holding all 21M dense embeddings
    in memory at once can OOM after hours of work. Build the FAISS index
    incrementally instead: keep passage metadata for lookup, but add each
    embedding batch directly to FAISS and then release the batch.
    """

    device = device or ("cuda" if torch.cuda.is_available() else "cpu")

    index = _create_faiss_index(device)
    ...
    for start in tqdm(range(0, len(texts), _PASSAGE_BATCH), desc="Embedding/indexing passages"):
        batch = texts[start : start + _PASSAGE_BATCH]

        embeddings = embed_passage_batch(batch, model, tokenizer, device)

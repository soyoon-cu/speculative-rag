...
def score_drafts_hf_batched(
    ...
    input_tensor = torch.tensor(padded).to(hf_model.device)
    ...
    for d_idx, (alpha_start, alpha_end, beta_start, beta_end, yes_idx) in enumerate(boundaries):
        log_probs_shifted = F.log_softmax(logits[d_idx, :-1, :], dim=-1) 
        target_ids = input_tensor[d_idx, 1:]                               
...
def score_drafts_vllm(
    ...
    # Submit m draft prompts at once
...
def main(
    ...
    for q_idx, record in enumerate(records):
        ...
        # PyTorch Profiler: profile one question
    ...
    # save summary block alongside records

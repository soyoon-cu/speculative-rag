...
MODEL_MISTRAL_7B = os.getenv("DRAFTER_MODEL_PATH", "mistralai/Mistral-7B-Instruct-v0.1")
...
class BatchedDrafter:
    def __init__(
        ...
        if use_bnb_nf4 and not self.use_bnb_nf4:
            logger.warning(
                "use_bnb_nf4=True ignored: bitsandbytes unavailable or device is not CUDA."
            )
        ...
        if self.use_vllm:
            ...
            self.vllm_llm, self.vllm_sampling = self._vllm.load_vllm(
                model_name, max_new_tokens, max_input_len, TEMPERATURE
            )
    ...
    def generate_transformers(self, prompts):
        ...
        logprobs_batch = DraftOutput.compute_seq_logprob(gen_out.scores, gen_ids)
        ...
        output_len_per_draft = (gen_ids != self.tokenizer.pad_token_id).sum(dim=1).cpu().tolist()
    ...
    def generate_drafts(self, question, subsets, profile_run=False, profile_dir="/profiler_traces"):
        ...
        docs_list = [s for _, s in valid_pairs]
        ...
        if self.use_vllm:
            if profile_run:
                nvtx.range_push(f"drafter.vllm_generate  m={m}")

            raw_outputs = self._vllm.generate_vllm(
                prompts, self.vllm_llm, self.vllm_sampling, profile_run
            )

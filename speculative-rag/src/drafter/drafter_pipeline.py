...
PROFILE_SAMPLE_INDEX = int(os.getenv("PROFILE_SAMPLE_INDEX", "75"))
...
class VerifierInput:
    ...
    drafts_tokens_in: int = 0

    drafts_tokens_out: int = 0
...
def process_one(sample, retriever, sampler, drafter, top_k, m, k_docs, profile_run, profile_dir):
    ...
    if profile_run:
        nvtx.range_push(f"pipeline.retrieve  qid={question_id}")
    ...
    if profile_run:
        nvtx.range_push(f"pipeline.sample  m={m}  k={k_docs}")
    ...
    if profile_run:
        nvtx.range_push(f"pipeline.draft  m={m}")

    t0 = sync_time()

    drafts = drafter.generate_drafts(
        question=question, subsets=subsets, profile_run=profile_run, profile_dir=profile_dir
    )
...
def load_pipeline(
    model_name, use_vllm, use_bnb_nf4, use_int8, index_path=INDEX_PATH, meta_path=META_PATH
):
...
def score_draft_outputs(results):
    ...
    for vi in results:
        ...
        any_hit = any(answer_in_response(vi.gold_answers, d.answer_draft) for d in vi.drafts)
    ...
    if results:
        ...
        pr.avg_total_ms = pr.avg_retrieval_ms + pr.avg_sampling_ms + pr.avg_drafting_ms
...
def run(
    ...
    for i, sample in enumerate(iter_samples(split="validation")):
        if test:
            ...
            logger.info(
                "[%d/%d] qid=%s  q=%s", i + 1, n_samples, sample.question_id, sample.question[:60]
            )
        ...
        vi = process_one(
            ...
            m=m,
            k_docs=k_docs,
            profile_run=profile_run and (i == PROFILE_SAMPLE_INDEX),
        ...
        if n_processed % log_every == 0:
            ...
            logger.info(
                "Processed %d | draft coverage so far %.2f%%",
                n_processed,
                progress.draft_coverage * 100,
            )

...
def build_prompt_and_boundaries(
    ...
    yes_idx     = len(full_token_ids) - 1   # "Yes" is always the last token

    return full_token_ids, alpha_start, alpha_end, beta_start, beta_end, yes_idx


# ---------------------------------------------------------------------------
# HF scoring — one draft per call
# ---------------------------------------------------------------------------

def score_drafts_hf_batched(
    hf_model,
    tokenizer,
    input_token_ids: list[list[int]],  # m lists of different lengths
    boundaries: list[tuple],            # (alpha_start, alpha_end, beta_start, beta_end, yes_idx) per draft
) -> list[tuple[float, float]]:
    """
    All m drafts for one question in a single forward pass.

    Right-padding is safe for causal models: padding tokens only appear after all
    span boundaries, so causal masking ensures they never affect the logits we read.
    """
    max_len = max(len(ids) for ids in input_token_ids)
    pad_id = tokenizer.pad_token_id
    padded = [ids + [pad_id] * (max_len - len(ids)) for ids in input_token_ids]
    input_tensor = torch.tensor(padded).to(hf_model.device)  # (m, max_len)

    with torch.no_grad():
        logits = hf_model(input_tensor).logits  # (m, max_len, V)

    results = []
    for d_idx, (alpha_start, alpha_end, beta_start, beta_end, yes_idx) in enumerate(boundaries):
        log_probs_shifted = F.log_softmax(logits[d_idx, :-1, :], dim=-1)  # (max_len-1, V)
        target_ids = input_tensor[d_idx, 1:]                               # (max_len-1,)
        token_log_probs = torch.gather(
            log_probs_shifted, 1, target_ids.unsqueeze(-1)
...
def score_draft_hf(
    ...
    # Shift: log_probs_shifted[i] = log P(token[i+1] | context up to i)
    log_probs_shifted = F.log_softmax(logits[:-1, :], dim=-1)  # (L-1, V)
    target_ids        = input_tensor[0, 1:]                     # (L-1,)
    token_log_probs   = torch.gather(
        log_probs_shifted, 1, target_ids.unsqueeze(-1)
    ).squeeze(-1).cpu().tolist()                               # list length L-1

    # ── ρ_SC — FIX 1: sum, no normalisation ──────────────────────────────────
    log_p_sc = 0.0
    for p in range(alpha_start, alpha_end):
        if p >= 1:
            log_p_sc += token_log_probs[p - 1]
...
# ---------------------------------------------------------------------------
# vLLM scoring — all m drafts for one question in one generate() call
# ---------------------------------------------------------------------------
...
def main(
    ...
    model_name:           str  = typer.Option("mistralai/Mistral-7B-Instruct-v0.1", help="Generalist verifier LM"),
    n_samples:            int  = typer.Option(100,                                  help="Questions to process (0 = all)"),
    ...
    # LOCAL_WEIGHTS_PATH = "/app/model_weights"
    ...
    if use_vllm:
        ...
        # prompt_logprobs=1 — vLLM includes the actual token's logprob in the
        # dict even when it isn't the top-1 ranked token
        ...
        if quant_config is not None:
            ...
            # BnB places quantized params on CUDA, but non-param buffers
            # (e.g. RoPE cos/sin cache) stay on CPU — move them now.
            _model_device = next(hf_model.parameters()).device
            for buf in hf_model.buffers():
                if buf.device.type == 'cpu':
                    buf.data = buf.data.to(_model_device)
        else:
            hf_model = AutoModelForCausalLM.from_pretrained(
                model_name,
                quantization_config=quant_config,
    ...
    for q_idx, record in enumerate(records):
        ...
        # ── Build all m prompts + stash boundaries ────────────────────────────
        ...
        # ── NVTX range — sits beside pipeline.retrieve/sample/draft in nsys ──
        nvtx.range_push(f"pipeline.verify qid={record.get('question_id', q_idx)}")
        t0 = sync_time()

        # ── PyTorch Profiler — FIRST question only, matches drafter pattern ───
        # FIX 7: profiler wraps the forward pass(es), not the outer question loop.
        #        with_stack=False matches the drafter's generate_with_profiler().
        is_first_question  = (q_idx == 75)
        ...
        if should_profile_hf:
            ...
            _prof = profile(
                ...
                record_shapes = True,
                profile_memory= True,
                with_stack    = False,          # matches drafter convention
        ...
        # ── Score ─────────────────────────────────────────────────────────────
        if use_vllm:
            sc_sr_pairs = score_drafts_vllm(
                llm, sampling_params, question_prompt_token_ids, drafts
            )
            ...
            boundaries = [
                (d["_alpha_start"], d["_alpha_end"], d["_beta_start"], d["_beta_end"], d["_yes_idx"])
                for d in drafts
            ]
            with record_function("verifier.select"):
                sc_sr_pairs = score_drafts_hf_batched(
                    hf_model, tokenizer, question_prompt_token_ids, boundaries
                )
            for d_idx, (log_p_sc, log_p_sr) in enumerate(sc_sr_pairs):
                d = drafts[d_idx]
                d["score_sc"]    = log_p_sc
        ...
        if should_profile_hf:
            ...
            console.print(
                f"[bold magenta]Verifier profiler trace → {profile_dir}[/bold magenta]"
            )
            # Print summary table (matches drafter's generate_with_profiler output)
        ...
        # ── EM — FIX 3: answer_in_response, same function as drafter ─────────
        ...
        # ── Token throughput — FIX 4 & FIX 8 ─────────────────────────────────
        # FIX 4: forward drafter token counts from JSON into each record
        # FIX 8: verifier_tokens_out = 0 (scoring only — no generation)
    ...
    summary = {
        ...
        # FIX 5 — p50/p95 per stage and end-to-end
    ...
    # ── FIX 9: save summary block alongside records ───────────────────────────
    ...
    console.print(f"  Exact Match (EM)         : {accuracy:.2f}%  ({correct_count}/{total_questions})")
    console.print("")
    console.print("  Latency (p50 / p95) [ms]")
    console.print(f"    retrieve               : {summary['p50_retrieve_ms']:8.1f}  /  {summary['p95_retrieve_ms']:.1f}")
    console.print(f"    sample                 : {summary['p50_sample_ms']:8.1f}  /  {summary['p95_sample_ms']:.1f}")
    console.print(f"    draft                  : {summary['p50_draft_ms']:8.1f}  /  {summary['p95_draft_ms']:.1f}")
    console.print(f"    verify                 : {summary['p50_verify_ms']:8.1f}  /  {summary['p95_verify_ms']:.1f}")
    console.print(f"    end-to-end             : {summary['p50_e2e_ms']:8.1f}  /  {summary['p95_e2e_ms']:.1f}")
    console.print("")
